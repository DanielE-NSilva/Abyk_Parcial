            ...
            connection.query('SELECT * FROM  producto WHERE nombreImagen = ? ', imgUrl, async (err, producto) => {
                if (producto.length >= 1)
                    ...
                    if (ext === '.png' || ext === '.jpg' || ext === '.jpeg' || ext === '.gif') {
                        ...
                        var data = [imgUrl + ext, req.body.name, req.body.descripcion, req.body.caracteristicas, req.body.referencia, req.body.cantidad, req.body.categoria];
                        req.getConnection((err, connection) => {
                            connection.query('INSERT INTO producto (nombreImagen,nombre,descripcion,caracteristicas,referencia,cantidad,categoria) value (?,?,?,?,?,?,?)', data, (err, producto) => {
                                Datos.Alert = {
                                    alert: true,
    ...
    req.getConnection((err, conn) => {
        conn.query('SELECT * FROM producto WHERE IdProducto = ?', [IdProducto], (err, producto) => {
            ...
            if(producto[0].Categoria == 1)
                producto[0].Categoria = "Femeninos"
            else
